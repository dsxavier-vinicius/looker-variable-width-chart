looker.plugins.visualizations.add({
  ...
  updateAsync: function (data, element, config, queryResponse, details, doneRendering) {
    alert("Custom visualization loaded ✅");
    console.log("🔍 Dados recebidos:", { data, config, queryResponse });
    ...
    if (element.clientWidth === 0 || element.clientHeight === 0) {
      console.warn("⚠️ Container with zero size, retrying...");
      setTimeout(() => this.updateAsync(data, element, config, queryResponse, details, doneRendering), 100);
      return;
    }
    ...
    const processedData = data.map(row => {
      const heightValue = parseFloat(row[yMeasure.name]?.value.replace(/[$,]/g, '')) || 0;
      const widthValue = parseFloat(row[widthMeasure.name]?.value.replace(/[$,]/g, '')) || 0;
      const item = {
    ...
    if (totalWidth <= 0) {
      this.addError({ title: 'Dados de largura inválidos', message: 'A soma da largura precisa ser maior que zero.' });
      return;
    }
    ...
    this._svg.attr('width', element.clientWidth).attr('height', element.clientHeight);
    ...
    const drawableWidth = chartWidth - totalSpacing;

    const xScale = d3.scaleLinear().domain([0, totalWidth]).range([0, drawableWidth]);
    const yScale = d3.scaleLinear().domain([0, yMax > 0 ? yMax : 1]).range([chartHeight, 0]).nice();

    if (config.show_y_axis) {
      g.append('g')
        .attr('class', 'chart-axis')
        .call(d3.axisLeft(yScale).ticks(5).tickFormat(d3.format(config.y_axis_format || ',')));
    }
    ...
    g.selectAll('.bar-rect')
      ...
      .attr('x', d => {
        const x = currentX;
        currentX += xScale(d.widthValue) + spacing;
        return x;
      })
      ...
      .on('mouseover', (event, d) => {
        const tooltip = `
          ...
          <div class="tooltip-metric"><span class="tooltip-metric-label">${yMeasure.label}</span><span class="tooltip-metric-value">${LookerCharts.Utils.textForCell(d._cells.y)}</span></div>
          <div class="tooltip-metric"><span class="tooltip-metric-label">${widthMeasure.label}</span><span class="tooltip-metric-value">${LookerCharts.Utils.textForCell(d._cells.width)}</span></div>
        `;
        this._tooltip.html(tooltip).style('display', 'block');
      })
      .on('mousemove', event => {
    ...
    if (config.show_labels) {
      ...
      g.selectAll('.bar-label')
        ...
        .attr('x', d => {
          const x = currentX + xScale(d.widthValue) / 2;
          currentX += xScale(d.widthValue) + spacing;
          return x;
        })
